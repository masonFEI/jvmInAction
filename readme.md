...
> - 在动态编译同步块的时候，JIT编译期可以借助逃逸分析来<span style="color:red">
    ...
    >
    如果没有，那么JIT编译器在编译这个同步块的时候就会取消对这部分代码的同步。这样就能大大提高并发性和性能，这个取消同步的过程就叫同步省略，也叫<span style="color:red">
    锁消除</span>。
>
...
> 这点就是导致GC进行时必须STW的一个重要原因；几十是号称（几乎）不会发生停顿的CMS收集器中，枚举根节点时也是必须要停顿的。
###### finalization机制
...
> 
> 由于finalize()方法的存在，虚拟机的对象一般处于三种可能得状态。
> 1. 可触及的：从根节点开始,可以到达这个对象。
> 2. 可复活的：对象的所有引用都被释放，但是对象有可能在finalize()中复活。
> 3. 不可触及的：对象的所有引用都被释放，并且没有复活，那么就会进入不可触及状态，不可触及的对象不可能被复活，因为finalize()只会被调用一次。
> 只有对象不可触及时才可以被回收。
